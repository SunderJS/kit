...
export class SessionManager<SessionDataType> {
  ...
  private getSetCookieHeaderValue(sessionToken: string, maxAge: number, secure: boolean) {
    return `${this.getCookieName(secure)}=${sessionToken}; HttpOnly; SameSite=strict; ${
      secure ? "Secure; " : ""
    }Max-Age=${maxAge}; Path=/`;
  }

  private getDeleteSetCookieHeaderValue(secure: boolean) {
    return `${this.getCookieName(secure)}=""; HttpOnly; SameSite=strict; ${
      secure ? "Secure; " : ""
  ...
  public async getSessionFromRequest(
    request: Request
  ): Result<SessionDataType, "cookie_not_found" | "expired" | "invalid"> {
